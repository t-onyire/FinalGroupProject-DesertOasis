...
window.addEventListener('keydown', e => {
  ...
  isNight = !isNight;
  if (isNight) {
    skyDome.material.map = sunsetSkyTex;
    scene.fog.color.setHex(0x5c1a00);
    sunLight.intensity=0.6; sunLight.color.setHex(0xff4400);
    ambientLight.intensity=0.25; ambientLight.color.setHex(0x4a1a00);
    hemiLight.intensity=0.1;
    waterUniforms.uWaterColor.value.setHex(0x0a4a3a);
    if (campfireOn) campfireLight.intensity=8;
  } else {
    skyDome.material.map = daySkyTex;
    scene.fog.color.setHex(0xd4a84b);
    sunLight.intensity=2.5; sunLight.color.setHex(0xffd27a);
    ambientLight.intensity=0.7; ambientLight.color.setHex(0xffe0b0);
    hemiLight.intensity=0.4;
    waterUniforms.uWaterColor.value.setHex(0x1a9e8f);
    if (campfireOn) campfireLight.intensity=5;
  }
